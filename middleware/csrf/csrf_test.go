...
func Test_CSRF(t *testing.T) {
	...
	methods := [4]string{woody.MethodGet, woody.MethodHead, woody.MethodOptions, woody.MethodTrace}
	...
	for _, method := range methods {
		...
		ctx.Request.Reset()
		ctx.Response.Reset()
		ctx.Request.Header.SetMethod(woody.MethodPost)
		h(ctx)
		...
		ctx.Request.Header.SetMethod(method)
		h(ctx)
		token := string(ctx.Response.Header.Peek(woody.HeaderSetCookie))
...
func Test_CSRF_From_Form(t *testing.T) {
	...
	// Invalid CSRF token
	ctx.Request.Header.SetMethod(woody.MethodPost)
	ctx.Request.Header.Set(woody.HeaderContentType, woody.MIMEApplicationForm)
	h(ctx)
	utils.AssertEqual(t, 403, ctx.Response.StatusCode())
...
func Test_CSRF_From_Param(t *testing.T) {
	...
	csrfGroup.Post("/", func(c *woody.Ctx) error {
		return c.SendStatus(woody.StatusOK)
	})
...
func Test_CSRF_From_Cookie(t *testing.T) {
	...
	ctx.Request.Header.SetMethod(woody.MethodPost)
	ctx.Request.SetRequestURI("/")
	ctx.Request.Header.Set(woody.HeaderCookie, "csrf="+utils.UUID()+";")
	h(ctx)
	utils.AssertEqual(t, 403, ctx.Response.StatusCode())
	...
	ctx.Response.Reset()
	ctx.Request.Header.SetMethod(woody.MethodPost)
	ctx.Request.Header.Set(woody.HeaderCookie, "csrf="+token+";")
	ctx.Request.SetRequestURI("/")
	h(ctx)
	utils.AssertEqual(t, 200, ctx.Response.StatusCode())
	utils.AssertEqual(t, "OK", string(ctx.Response.Body()))
...
func Test_CSRF_From_Custom(t *testing.T) {
	...
	// Invalid CSRF token
	ctx.Request.Header.SetMethod(woody.MethodPost)
	ctx.Request.Header.Set(woody.HeaderContentType, woody.MIMETextPlain)
	h(ctx)
	utils.AssertEqual(t, 403, ctx.Response.StatusCode())
...
func Test_CSRF_ErrorHandler_InvalidToken(t *testing.T) {
	...
	errHandler := func(ctx *woody.Ctx, err error) error {
...
// TODO: use this test case and make the unsafe header value bug from https://github.com/ximispot/woody/issues/2045 reproducible and permanently fixed/tested by this testcase
...
// 	getReq := httptest.NewRequest(woody.MethodGet, "/", nil)
// 	getReq.Header.Set(HeaderName, token)
// 	resp, err = app.Test(getReq)

// 	getReq = httptest.NewRequest(woody.MethodGet, "/test", nil)
// 	getReq.Header.Set("X-Requested-With", "XMLHttpRequest")
// 	getReq.Header.Set(woody.HeaderCacheControl, "no")
// 	getReq.Header.Set(HeaderName, token)

// 	resp, err = app.Test(getReq)

// 	getReq.Header.Set(woody.HeaderAccept, "*/*")
// 	getReq.Header.Del(HeaderName)
// 	resp, err = app.Test(getReq)

// 	postReq := httptest.NewRequest(woody.MethodPost, "/", nil)
