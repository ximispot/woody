...
func Test_ETag_SkipError(t *testing.T) {
	...
	resp, err := app.Test(httptest.NewRequest(woody.MethodGet, "/", nil))
	utils.AssertEqual(t, nil, err)
	utils.AssertEqual(t, woody.StatusForbidden, resp.StatusCode)
}
...
func Test_ETag_NotStatusOK(t *testing.T) {
	...
	app.Get("/", func(c *woody.Ctx) error {
		return c.SendStatus(woody.StatusCreated)
	})

	resp, err := app.Test(httptest.NewRequest(woody.MethodGet, "/", nil))
	utils.AssertEqual(t, nil, err)
	utils.AssertEqual(t, woody.StatusCreated, resp.StatusCode)
}
...
func testETagNewEtag(t *testing.T, headerIfNoneMatch, matched bool) { //nolint:revive // We're in a test, so using bools as a flow-control is fine
	...
	if headerIfNoneMatch {
		...
		req.Header.Set(woody.HeaderIfNoneMatch, etag)
	}
	...
	if !headerIfNoneMatch || !matched {
		utils.AssertEqual(t, woody.StatusOK, resp.StatusCode)
		utils.AssertEqual(t, `"13-1831710635"`, resp.Header.Get(woody.HeaderETag))
		return
	}

	if matched {
		utils.AssertEqual(t, woody.StatusNotModified, resp.StatusCode)
...
func testETagWeakEtag(t *testing.T, headerIfNoneMatch, matched bool) { //nolint:revive // We're in a test, so using bools as a flow-control is fine
	...
	if !headerIfNoneMatch || !matched {
		utils.AssertEqual(t, woody.StatusOK, resp.StatusCode)
		utils.AssertEqual(t, `W/"13-1831710635"`, resp.Header.Get(woody.HeaderETag))
		return
	}
...
func testETagCustomEtag(t *testing.T, headerIfNoneMatch, matched bool) { //nolint:revive // We're in a test, so using bools as a flow-control is fine
	...
	app.Get("/", func(c *woody.Ctx) error {
		c.Set(woody.HeaderETag, `"custom"`)
		if bytes.Equal(c.Request().Header.Peek(woody.HeaderIfNoneMatch), []byte(`"custom"`)) {
	...
	if !headerIfNoneMatch || !matched {
		utils.AssertEqual(t, woody.StatusOK, resp.StatusCode)
		utils.AssertEqual(t, `"custom"`, resp.Header.Get(woody.HeaderETag))
		return
	}
...
func Test_ETag_CustomEtagPut(t *testing.T) {
	...
	app.Put("/", func(c *woody.Ctx) error {
		c.Set(woody.HeaderETag, `"custom"`)
		if !bytes.Equal(c.Request().Header.Peek(woody.HeaderIfMatch), []byte(`"custom"`)) {
			return c.SendStatus(woody.StatusPreconditionFailed)
		}
		return c.SendString("Hello, World!")
	})

	req := httptest.NewRequest(woody.MethodPut, "/", nil)
	req.Header.Set(woody.HeaderIfMatch, `"non-match"`)
	resp, err := app.Test(req)
	utils.AssertEqual(t, nil, err)
	utils.AssertEqual(t, woody.StatusPreconditionFailed, resp.StatusCode)
}
...
func Benchmark_Etag(b *testing.B) {
	...
	utils.AssertEqual(b, `"13-1831710635"`, string(fctx.Response.Header.Peek(woody.HeaderETag)))
}
